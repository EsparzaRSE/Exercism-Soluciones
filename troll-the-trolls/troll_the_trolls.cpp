    ...
    bool permission_check(Action action, AccountStatus status){
        switch (action){
            case Action::read:
                ...
                break;
            case Action::write:
                if(status != AccountStatus::guest) return true;
                else return false;
                break;
            case Action::remove:
                if(status == AccountStatus::mod) return true;
                else return false;
                break;
            default:
                return false;
...
// of type `AccountStatus` and returns a `bool`.
    bool valid_player_combination(AccountStatus player1, AccountStatus player2){
        if(player1 == AccountStatus::guest || player2 == AccountStatus::guest) return false;
            ...
            || (player1 != AccountStatus::troll && player2 == AccountStatus::troll)) return false;
        else return true;
    }
...
// account has a strictly higher priority than the second.
    ...
    bool has_priority(AccountStatus account1, AccountStatus account2){
        if(static_cast<int>(account1) > static_cast<int>(account2)) return true;
        else return false;
    }
}  // namespace hellmath